...
def sum_int (device_profile, index_list):
  int_list = [(idx+1) * 5237 * int(device_profile[idx]) for idx in index_list if device_profile[idx].isdigit()]
  return sum(int_list)

def sum_float (device_profile, index_list):
  float_list = [(idx+1) * 4999 * float(device_profile[idx]) for idx in index_list]
