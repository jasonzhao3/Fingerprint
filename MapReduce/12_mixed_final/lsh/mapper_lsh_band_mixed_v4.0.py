...
for i in range(len(value_map[14])):
  hid_map_1[value_map[14][i]] = perm_map_1[14][i]
  hid_map_2[value_map[14][i]] = perm_map_2[14][i]
  hid_map_3[value_map[14][i]] = perm_map_3[14][i]


'''
  hash set - min hash
'''
def getSignature (feature_set, value_map, index): 
    #print index    
    full_set = value_map[index]
    perm_1 = perm_map_1[index]
    perm_2 = perm_map_2[index]
    perm_3 = perm_map_3[index]
    comb_map_1 = {}
    comb_map_2 = {}
    comb_map_3 = {}
    if (index) == 14: #hid
        if feature_set[0] == "0":
            return str(random.randint(1, len(full_set)))
        else:
            sig1 = min(hid_map_1[feature] for feature in feature_set)
            sig2 = min(hid_map_2[feature] for feature in feature_set)
            sig3 = min(hid_map_3[feature] for feature in feature_set)
            return str(sig1 + sig2 + sig3)
    for i in range(len(full_set)):
        comb_map_1[perm_1[i]] = i
        comb_map_2[perm_2[i]] = i
        comb_map_3[perm_3[i]] = i 
    ...
    if len(feature_set) == 1:        
        if (feature_set[0] == "0" and (index == 4 or index == 5 or index == 22)) or feature_set[0].lower() == "null" or feature_set[0].lower() == "na" or feature_set[0].lower() == "n/a":
            signature1 = random.randint(0, len(full_set))
            signature2 = random.randint(0, len(full_set))
            signature3 = random.randint(0, len(full_set))
            return str(signature1 + signature2 + signature3)    


    while signature1 < len(perm_1):
        val = full_set[comb_map_1[signature1]]
        if val in feature_set:
            break
        else:
            signature1 += 1
            
    while signature2 < len(perm_2):
        val = full_set[comb_map_2[signature2]]
        if val in feature_set:
            break
        else:
            signature2 += 1
            
    while signature3 < len(perm_3):
        val = full_set[comb_map_3[signature3]]
        if val in feature_set:
            break
        else:
            signature3 += 1
  ...
  for key, band_idx_list in full_band_idx.iteritems():
    ...
    for i in band_idx_list:
      if i in majority_idx:
        ...
        band_attrs.append(getSignature(device_profile[i].split('|'), value_map, i))
      else:
