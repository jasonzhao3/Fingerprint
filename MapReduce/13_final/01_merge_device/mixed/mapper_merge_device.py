...
def get_signature (feature_set, search_map_list, index): 
    ...
    for search_map in search_map_list:
      ...
      for feature in feature_set:
        try:
          feature_idx = search_dict[feature]
        except (KeyError):
          feature_idx = randint(0, len(search_dict)-1)
...
def hash_full_profile (device_profile):
  ...
  hash_attrs = []
  for idx in full_profile:
    if idx in majority_idx:
        hash_attrs.append(device_profile[idx])
    elif idx in set_idx:
        hash_attrs.append(get_signature(device_profile[idx].split('|'), search_map_list, idx))

  hash_attrs.append(device_profile[-1])
